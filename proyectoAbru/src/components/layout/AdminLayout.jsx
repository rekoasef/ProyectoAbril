...
import { LogOut, Image, Briefcase, LayoutDashboard, ChevronDown, ChevronUp, Menu, X } from 'lucide-react';
  ...
  return (
    <div className="flex h-screen bg-gray-100">
        ...
        <nav className="flex-1 px-2 py-4 space-y-1">
          {navItems.map(item => (
            <NavLink
              key={item.text}
              to={item.to}
              end // 'end' es importante para que "/" no coincida con "/admin/portfolio"
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Navbar Superior (para móvil y puede tener breadcrumbs o user info en desktop) */}
        ...
        <main className="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100 p-6 md:p-10">
          <Outlet /> {/* Aquí se renderizarán las páginas de admin (Dashboard, Portfolio, etc.) */}
        </main>
      </div>
    </div>
  );
